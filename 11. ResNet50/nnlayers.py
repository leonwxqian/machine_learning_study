...
def zero_padding(input, padding=(1, 1), data_format=None):
    return tf.keras.layers.ZeroPadding2D(padding=(1, 1), data_format=None)(input)


def conv2d_bn(input, filter_count, kernel_row, kernel_col, padding='SAME', strides=1, kernel_regularizer='none', l2_value=0.0005, activation='none', kernel_initializer='glorot_normal', axes=0):
    ...
    bn = batch_normalization(conv, axes)
...
def batch_normalization(inputs, axes=0):
    mean, variance = tf.nn.moments(inputs, axes=axes)
