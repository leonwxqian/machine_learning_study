...
def inception_v3(input, f1, f1_w, f2, f2_w, f3, f3_w, f4, f4_w, f5, f5_w, f6, f6_w, f7, f7_w, f8, f8_w, f3_h=8888,
                 f4_h=8888):
    if (f3_h == 8888):
        f3_h = f3_w
    if (f4_h == 8888):
...
def inception_v3_b(input, f1, f1_w, f2, f2_w, f3, f3_w, f3_h, f4, f4_w, f4_h, f5, f5_w, f6, f6_w, f6_h, f7, f7_w, f7_h,
                   f8, f8_w, f8_h, f9, f9_w, f9_h, f10, f10_w):
    branch1x1 = nnlayers.conv2d_bn(input, f1, f1_w, f1_w)
...
def build_model():
    ...
    x = inception_v3_b(x, 192, 1, 128, 1, 128, 1, 7, 192, 7, 1, 128, 1, 128, 7, 1, 128, 1, 7, 128, 7, 1, 192, 1, 7, 192,
                       1)
    x = inception_v3_b(x, 192, 1, 160, 1, 160, 1, 7, 192, 7, 1, 160, 1, 160, 7, 1, 160, 1, 7, 160, 7, 1, 192, 1, 7, 192,
                       1)
    x = inception_v3_b(x, 192, 1, 160, 1, 160, 1, 7, 192, 7, 1, 160, 1, 160, 7, 1, 160, 1, 7, 160, 7, 1, 192, 1, 7, 192,
                       1)
    x = inception_v3_b(x, 192, 1, 192, 1, 192, 1, 7, 192, 7, 1, 192, 1, 192, 7, 1, 192, 1, 7, 192, 7, 1, 192, 1, 7, 192,
                       1)

    # mixed 8: 8 x 8 x 1280
    branch3x3 = nnlayers.conv2d_bn(x, 192, 1, 1)
    branch3x3 = nnlayers.conv2d_bn(branch3x3, 320, 1, 1,
    ...
    branch7x7x3 = nnlayers.conv2d_bn(
        branch7x7x3, 192, 3, 3, strides=2, padding='same')

    branch_pool = nnlayers.max_pooling(x, 3, strides=2, padding='same')
    x = tf.concat(
    ...
    logits = nnlayers.dense(x, 100, kernel_regularizer='none', activation='none')
    return logits
...
train_step = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=learning_rate).minimize(cross_entropy, global_step=global_step)
