...
def inception(input, f1, f2, f3, f4 ,f5 ,f6):
    ...
    inception_5x5 = nnlayers.conv_2d(inception_5x5_reduce, filter_count=f5, kernel_size=5, strides=1)
    ...
    output = tf.concat([inception_1x1, inception_3x3, inception_5x5,inception_pool_proj],axis=3)
    return output
...
def build_model():
    ...
    inception_3a_output = inception(pool2_3x3_s2, 64, 96, 128, 16, 32, 32)

    inception_3b_output = inception(inception_3a_output, 128, 128, 192, 32, 96, 64)

    pool3_3x3_s2 = nnlayers.max_pooling(inception_3b_output, 3, 2)

    inception_4a_output = inception(pool3_3x3_s2, 192, 96, 208, 16, 48, 64)

    inception_4b_output = inception(inception_4a_output, 160, 112, 224, 24, 64, 64)

    inception_4c_output = inception(inception_4b_output, 128, 128, 256, 24, 64, 64)

    inception_4d_output = inception(inception_4c_output, 112, 144, 288, 32, 64, 64)

    inception_4e_output = inception(inception_4d_output, 256, 160, 320, 32, 128, 128)

    pool4_3x3_s2 = nnlayers.max_pooling(inception_4e_output, 3, 2)

    inception_5a_output = inception(pool4_3x3_s2, 256, 160, 320, 32, 128, 128)

    inception_5b_output = inception(inception_5a_output, 384, 192, 384, 48, 128, 128)
